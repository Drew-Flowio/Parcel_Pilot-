-- ================================================================
-- AppFolio push state on thread-day rollups
-- ----------------------------------------------------------------
-- The rollup table is the source the AppFolio push worker consumes.
-- We add two columns:
--
--   appfolio_note_id    text  — AppFolio-side note id once we've
--                               successfully created it. Re-pushes
--                               should PATCH this id (no duplicates).
--   last_push_reason    text  — most-recent policy decision, e.g.
--                               "pushed", "skip:low-signal-single-message",
--                               "queue:day-still-active", "retry:network".
--                               Surface in the dashboard for visibility.
--
-- Push semantics in one paragraph:
--   `synced_to_appfolio_at` IS NULL  ⇒ row is up for re-evaluation.
--   The push worker decides PUSH / SKIP / QUEUE / RETRY:
--     PUSH  → succeeded → set synced_to_appfolio_at = now(),
--                            appfolio_note_id = <af id>,
--                            last_push_reason  = 'pushed'.
--     SKIP  → policy says nothing to send → set synced_to_appfolio_at = now(),
--                            last_push_reason = 'skip:<reason>'.
--                            Rolls re-fire only when content changes (rollup
--                            function resets synced_to_appfolio_at = NULL).
--     QUEUE → soft hold (e.g. day still active) → leave synced_to_appfolio_at
--                            NULL, set last_push_reason = 'queue:<reason>'.
--                            Next tick re-evaluates.
--     RETRY → AppFolio 429/5xx/network → leave synced_to_appfolio_at NULL,
--                            set last_push_reason = 'retry:<reason>'.
--
-- Critically, the rollup function PRESERVES `appfolio_note_id` across
-- re-runs (it's our linkage to the AF row) but CLEARS `last_push_reason`
-- (the policy must re-decide given new content).
-- ================================================================

alter table public.skywalk_thread_day_rollups
  add column if not exists appfolio_note_id text;

alter table public.skywalk_thread_day_rollups
  add column if not exists last_push_reason text;

comment on column public.skywalk_thread_day_rollups.appfolio_note_id is
  'AppFolio-side note id once successfully pushed. Subsequent pushes for the same rollup PATCH this id (idempotent re-sync).';
comment on column public.skywalk_thread_day_rollups.last_push_reason is
  'Most recent push-policy decision: "pushed", "skip:<reason>", "queue:<reason>", or "retry:<reason>". Cleared whenever the rollup is re-aggregated.';

-- Dashboard / debugging: "show me everything currently held back"
create index if not exists skywalk_thread_day_rollups_push_reason_idx
  on public.skywalk_thread_day_rollups (last_push_reason)
  where last_push_reason is not null
    and last_push_reason <> 'pushed';

-- ================================================================
-- Rebuild skywalk_rollup_thread_days() — preserve appfolio_note_id,
-- clear last_push_reason on every content change.
-- ================================================================
...
begin
  ...
  upsert as (
    ...
    on conflict (skywalk_conversation_id, rollup_date) do update set
      ...
      -- Push state: clear sync timestamp + decision; PRESERVE appfolio_note_id
      -- so the worker PATCHes the existing AF row instead of creating a dup.
      synced_to_appfolio_at = null,
      last_push_reason      = null
...
comment on function public.skywalk_rollup_thread_days(timestamptz, text, text) is
  'Re-aggregate skywalk_messages into skywalk_thread_day_rollups. Resets summary_version=1, synced_to_appfolio_at=NULL, and last_push_reason=NULL on every change. Preserves appfolio_note_id so re-pushes PATCH the existing AppFolio note.';
