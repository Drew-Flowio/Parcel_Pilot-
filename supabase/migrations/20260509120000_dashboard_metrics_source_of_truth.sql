-- Dashboard metrics: one RPC = one source of truth (no partial JS sums).
-- Fixes:
--   * aggregate "market value" was sum(top 1000 portfolio_groups) — wrong.
--   * top-target threshold drifted from lead_segments (top_500_pm uses 60).
--   * LLC % mixed portfolio-group counts with inconsistent denominators.
--
-- All currency here is Hennepin assessor total estimated market value
-- (MKT_VAL_TOT per parcel), NOT monthly/annual rent or income.
...
comment on function public.parcel_pilot_dashboard_metrics is
  'Single JSON blob for Parcel Pilot homepage: parcel-grain aggregates from parcels_raw + parcels_intel + portfolio_groups. Values are assessed market (MKT_VAL_TOT), not rent/income; top-target floor follows lead_segments.top_500_pm.min_score_v2.';
