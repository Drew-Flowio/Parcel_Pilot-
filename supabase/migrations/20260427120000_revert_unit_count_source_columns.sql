-- Revert remote migration unit_count_source_columns_and_function:
-- drop helper + optional columns, restore parcels_raw + public.parcels to
-- the shape from 20260416120000_expand_unit_count_parcels_raw.

drop function if exists public.parcel_pilot_unit_int_from_text(text);

alter table public."Hennepin Parcels" drop column if exists "NUM_UNITS";
alter table public."Hennepin Parcels" drop column if exists "NO_UNITS";
alter table public."Hennepin Parcels" drop column if exists "NBR_UNITS";
alter table public."Hennepin Parcels" drop column if exists "RES_UNITS";
