-- Parcel Pilot: materialize the expensive "raw" Hennepin filter so public.parcels
-- does not seq-scan ~448k rows on every API request (avoids statement_timeout).
-- Refresh after bulk updates to "Hennepin Parcels":
...
comment on materialized view public.parcels_raw is
  'Filtered Hennepin multifamily slice (~31k rows). Refresh after GIS import: REFRESH MATERIALIZED VIEW CONCURRENTLY public.parcels_raw;';
